...
@router.post(
    "/swipe-recommend", response_model=schemas.swipe_card.SwipeCardMessage
    #"/swipe-recommend"
)
def get_recommendation(
    *,
    ...
    recommended_member_id_list = recommendation_context.recommend(
        recommend_in.member_id, recommend_in.room_uuid
    )

    recommend_card_list = []
    for rcmd_member_id in recommended_member_id_list:
        # get find tag, self tag by member_id and room_uuid
        self_tag = jsonable_encoder(crud.swipe_card.get_self_tag_by_member_id_and_room_uuid(db=db, member_id=str(rcmd_member_id), room_uuid=recommend_in.room_uuid))
        self_tag_text = [ t["tag_text"] for t in self_tag]
        find_tag = jsonable_encoder(crud.swipe_card.get_find_tag_by_member_id_and_room_uuid(db=db, member_id=str(rcmd_member_id), room_uuid=recommend_in.room_uuid))
        find_tag_text = [ t["tag_text"] for t in find_tag]

        # get member profile (image, name)
        user = crud.swipe_card.get_user_profile_by_member_id(db=db, member_id=rcmd_member_id)
        image = user.image
        name = user.name
        
        # make a recommendation list
        swipe_card = {"member_id":recommend_in.member_id,
